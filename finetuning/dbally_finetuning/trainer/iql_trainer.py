...
from dbally_finetuning.data.dataset import IQLDataset
...
class IQLTrainer:
    ...
    def _prepare_dataset(self) -> None:
        ...
        for split, path in self.config.dataset.data_files.items():
            self.datasets[split] = self._processor.process(IQLDataset.from_json_file(data_dir / path))

    def _set_processor(self) -> None:
        self._processor: Preprocessor = Preprocessor(self._tokenizer)

    def _set_tokenizer(self) -> None:
        self._tokenizer: PreTrainedTokenizer = AutoTokenizer.from_pretrained(
            self.config.model.name, trust_remote_code=True
        )
    ...
    def _set_trainer(self) -> None:
        ...
        if self.config.use_lora:
            peft_params = LoraConfig(target_modules=self.config.model.lora_target_modules, **self.config.lora_params)
        if self.config.use_qlora:
