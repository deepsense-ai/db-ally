...
class BaseStructuredView(BaseView):
    ...
    async def ask(
        ...
        for _ in range(n_retries + 1):
            iql, prompt_template = await iql_generator.generate_iql(
                question=query,
                filters=filters,
                examples=examples,
            ...
            try:
                parsed_iql = await IQLQuery.parse(iql, filters, event_tracker=event_tracker)
                await self.apply_filters(parsed_iql)
                break
